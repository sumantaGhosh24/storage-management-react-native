...
const OthersFileScreen = () => {
  ...
  const types = getFileTypesParams("others");

  const {data: others, refetch} = useAppwrite(() =>
    getFiles({
      types,
      limit: LIMIT,
      page: filter.page,
  ...
  const typedOthers = others as unknown as OthersParams;
  ...
  return (
    <ScrollView>
      <View className="flex flex-row items-center gap-5 p-5 bg-blue-600">
        <Text className="text-3xl font-bold text-white">All Others Files</Text>
        <IconButton
      ...
      <View className="dark:bg-black px-5 min-h-screen">
        <View>
          ...
          <View>
            <View>
              {typedOthers.total > 0 ? (
                typedOthers?.files.map((file) => (
                  <FileThumbnail file={file} key={file.$id} />
                ))
              ) : (
                <EmptyState
                  title="No others found"
            ...
            {typedOthers.total > 0 && (
              <View className="flex flex-row items-center gap-3 my-5 flex-wrap">
                {[...Array(Math.ceil(typedOthers.total / LIMIT))]
                  .fill(0)
                  .map((c, i) => (
                    <TouchableOpacity
                      key={i + 1}
...
export default OthersFileScreen;
