...
const DocumentsFileScreen = () => {
  ...
  const types = getFileTypesParams("documents");

  const {data: documents, refetch} = useAppwrite(() =>
    getFiles({
      types,
      limit: LIMIT,
      page: filter.page,
  ...
  const typedDocuments = documents as unknown as DocumentParams;
  ...
  return (
    <ScrollView>
      <View className="flex flex-row items-center gap-5 p-5 bg-blue-600">
        <Text className="text-3xl font-bold text-white">All Documents</Text>
        <IconButton
      ...
      <View className="dark:bg-black px-5 min-h-screen">
        <View>
          ...
          <View>
            <View>
              {typedDocuments.total > 0 ? (
                typedDocuments?.files.map((file) => (
            ...
            {typedDocuments.total > 0 && (
              <View className="flex flex-row items-center gap-3 my-5 flex-wrap">
                {[...Array(Math.ceil(typedDocuments.total / LIMIT))]
                  .fill(0)
                  .map((c, i) => (
                    <TouchableOpacity
                      key={i + 1}
...
export default DocumentsFileScreen;
